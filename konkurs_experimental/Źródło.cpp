...
zaimplementowane rozwiazania z wykladu:
    ...
    - wektor tau - WspÛ≥czynniki uczenia dla samo-adaptacji sigmy. Wyznaczane wg wzorÛw Schwefela.
dodatkowe rozwiazania spoza wykladu:
    - Mechanizm pÍdu - Obliczamy wektor momentum jako wyk≥adniczπ úredniπ kroczπcπ przesuniÍÊ centroidy
      i nastÍpnie dodajemy go do mutacji. DziÍki temu kumulujemy informacje o kierunku,
      w ktÛrym porusza siÍ populacja, co pozwala na szybsze przebywanie p≥askich obszarÛw i wyd≥uøanie kroku w dobrym kierunku.
    ...
    - Strategia RestartÛw IPOP (Increasing Population) - Gdy algorytm utknie, nastÍpuje restart ze zwiÍkszonπ populacjπ. 
    ...
    - Resetowanie lamdy gdy stanie sie zbyt duza - pomysl autorksi, jednak sotoswany w innych algorytmach optymalizacyjnych. 
      Zapobiegamy zbyt szybkiemu zuzyciu kredytow poprzez zbyt duza wartosc lambda
...
struct Config {
    ...
    double momentum_beta = 0.6; // Si≥a pÍdu
...
void enforceConstraints(Individual& ind, double low, double high) {
    for (size_t i = 0; i < ind.x.size(); ++i) {
        ...
        // Obciecie - nie odbijamy, øeby nie zawracaÊ do úrodka
...
// ================================================================
// OPERATORY (MOMENTUM ES)
// ================================================================

// Obliczanie centroidy (úredniej waøonej)
Individual calculateCentroid(const std::vector<Individual>& parents, const std::vector<double>& weights, int dim, int mu) {
    ...
    //c.fitness = 0;  //inicjalizujemy, ale ta wartosc nie bedzie uzwyaana
...
void mutate_with_momentum(Individual& ind, const std::vector<double>& momentum, const Config& cfg) {
    ...
    for (int d = 0; d < cfg.dim; ++d) {
        ...
        // Mutacja X z przesuniÍciem - mean_shift
        // x' = (x + momentum) + sigma * N(0,1)
        // Momentum "pchajπ" mutacjÍ w kierunku, w ktÛrym sz≥a populacja
...
// ================================================================
// ALGORYTM: MOMENTUM-DRIVEN ES (mu, lambda)
// ================================================================
std::vector<double> run_Momentum_ES(double (*objective)(const std::vector<double>&), Config cfg) {
    ...
    while (credits > 0) {
        ...
        while (credits > 0) {
            ...
            //OBLICZENIE CENTROIDY I P DU
            Individual currentCentroid = calculateCentroid(population, weights, cfg.dim, cfg.mu);

            // Aktualizacja wektora pÍdu - zgodnie z kierunkiem ruchu populacji
            for (int d = 0; d < cfg.dim; ++d) {
            ...
            //GENEROWANIE POTOMSTWA (Z uøyciem centroidy i pÍdu)
