...
zaimplementowane rozwiazania z wykladu:
    ...
    - wektor tau - Wspó³czynniki uczenia dla samo-adaptacji sigmy. Wyznaczane wg wzorów Schwefela.
dodatkowe rozwiazania spoza wykladu:
    - Mechanizm pedu - Obliczamy wektor momentum jako wykladnicza srednia kroczaca przesuniec centroidy
      i nastepnie dodajemy go do mutacji. Dziêki temu kumulujemy informacje o kierunku,
      w ktorym porusza siê populacja, co pozwala na szybsze przebywanie plaskich obszarow i wydluzanie kroku w dobrym kierunku.
    ...
    - Strategia Restartów IPOP (Increasing Population) - Gdy algorytm utknie, nastepuje restart ze zwiêkszon¹ populacja. 
      Zakladamy, ¿e jesli mala populacja zawiodla, to nalezy zrestartowac algorytm z wiêksza populacja, aby zwiekszyc szanse na globalna zbieznosc.
    ...
    while (credits > 0) {
        ...
        while (credits > 0) {
            ...
            for (int d = 0; d < cfg.dim; ++d) {
                ...
                momentum[d] = 0.5 * momentum[d] + 0.5 * move;
            }
...
int main() {
    ...
    int runs = 100; // Liczba uruchomieñ na konfiguracjê
    for (int n : dims) {
        Config cfg;
        cfg.dim = n;
        cfg.T_max = 10000 * n;
